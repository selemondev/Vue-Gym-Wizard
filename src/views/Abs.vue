...
const absExercises = ref([]);
const loading = ref(false);
const absExercise = async () => {
      loading.value = true;
    const absData = await useFetch("https://exercisedb.p.rapidapi.com/exercises/target/abs", exerciseOptions);
    absExercises.value = absData;
    loading.value = false;
}
absExercise();
...
<main>
    ...
    <div class="grid place-items-center mt-4">
       ...
       <div class="flex justify-center items-center flex-wrap" v-else>
        <div v-for="exercise in absExercises" :key="exercise.id" class="m-5">
