...
const shoulderExercises = ref([]);
const loading = ref(false);
const shoulderExercise = async () => {
    loading.value = true;
    const shoulderData = await useFetch("https://exercisedb.p.rapidapi.com/exercises/bodyPart/shoulders", exerciseOptions);
    shoulderExercises.value = shoulderData;
    loading.value = false;
}
shoulderExercise();
...
<main>
    ...
    <Loader/>
       ...
       <div class="flex justify-center items-center flex-wrap" v-else>
        <div v-for="exercise in shoulderExercises" :key="exercise.id" class="m-5">
